/*
**  Unit tests for ccunicode.h — UTF-8 ↔ UCS-4 codec.
**
...
/* ════════════════════════════════════════════════════════════════════════
**  ccunicode_to_codepoint  —  UTF-8 → UCS-4
** ════════════════════════════════════════════════════════════════════════ */
...
void test_decode_ascii(void) {
  ...
  /* single byte */
  n = ccunicode_to_codepoint("A", 1, &val);
  T(n == 1 && val == 0x41);

  /* null byte (U+0000 is valid) */
  n = ccunicode_to_codepoint("\x00", 1, &val);
  T(n == 1 && val == 0x00);

  /* boundary: last ASCII */
  n = ccunicode_to_codepoint("\x7F", 1, &val);
  T(n == 1 && val == 0x7F);

  /* first non-ASCII is not 1-byte */
  n = ccunicode_to_codepoint("\x80", 1, &val);
  T(n == 0);  /* invalid */
}
...
void test_decode_2byte(void) {
  ...
  /* U+00A2 (¢) = C2 A2 */
  n = ccunicode_to_codepoint("\xC2\xA2", 2, &val);
  T(n == 2 && val == 0xA2);

  /* U+00A9 (©) = C2 A9 */
  n = ccunicode_to_codepoint("\xC2\xA9", 2, &val);
  T(n == 2 && val == 0xA9);

  /* boundary: min 2-byte U+0080 = C2 80 */
  n = ccunicode_to_codepoint("\xC2\x80", 2, &val);
  T(n == 2 && val == 0x80);

  /* boundary: max 2-byte U+07FF = DF BF */
  n = ccunicode_to_codepoint("\xDF\xBF", 2, &val);
  T(n == 2 && val == 0x7FF);
}
...
void test_decode_3byte(void) {
  ...
  /* U+4E2D (中) = E4 B8 AD */
  n = ccunicode_to_codepoint("\xE4\xB8\xAD", 3, &val);
  T(n == 3 && val == 0x4E2D);

  /* U+6587 (文) = E6 96 87 */
  n = ccunicode_to_codepoint("\xE6\x96\x87", 3, &val);
  T(n == 3 && val == 0x6587);

  /* U+00E9 (é) = C3 A9 (actually 2-byte — verify 3-byte boundary) */
  /* boundary: min 3-byte U+0800 = E0 A0 80 */
  n = ccunicode_to_codepoint("\xE0\xA0\x80", 3, &val);
  T(n == 3 && val == 0x800);
  ...
  n = ccunicode_to_codepoint("\xEF\xBF\xBF", 3, &val);
  T(n == 3 && val == 0xFFFF);
}
...
void test_decode_4byte(void) {
  ...
  /* U+1F600 (😀) = F0 9F 98 80 */
  n = ccunicode_to_codepoint("\xF0\x9F\x98\x80", 4, &val);
  T(n == 4 && val == 0x1F600);

  /* U+2070E (𠜎) = F0 A0 9C 8E */
  n = ccunicode_to_codepoint("\xF0\xA0\x9C\x8E", 4, &val);
  T(n == 4 && val == 0x2070E);

  /* boundary: min 4-byte U+10000 = F0 90 80 80 */
  n = ccunicode_to_codepoint("\xF0\x90\x80\x80", 4, &val);
  T(n == 4 && val == 0x10000);

  /* boundary: max U+10FFFF = F4 8F BF BF */
  n = ccunicode_to_codepoint("\xF4\x8F\xBF\xBF", 4, &val);
  T(n == 4 && val == 0x10FFFF);

  /* beyond max: F4 90 80 80 → U+110000 (first byte valid but result > 0x10FFFF) */
  n = ccunicode_to_codepoint("\xF4\x90\x80\x80", 4, &val);
  T(n == 0);

  /* beyond max: F4 BF BF BF → even larger */
  n = ccunicode_to_codepoint("\xF4\xBF\xBF\xBF", 4, &val);
  T(n == 0);
}
...
void test_decode_multi(void) {
  ...
  n = ccunicode_to_codepoint(s, 6, &val);
  T(n == 3 && val == 0x4E2D);
  total += n;

  n = ccunicode_to_codepoint(s + total, 6 - total, &val);
  T(n == 3 && val == 0x6587);
  total += n;
...
void test_decode_overlong(void) {
  ...
  T(ccunicode_to_codepoint("\xC0\x80", 2, &val) == 0);

  /* 2-byte overlong: C1 BF would encode U+007F */
  T(ccunicode_to_codepoint("\xC1\xBF", 2, &val) == 0);

  /* 3-byte overlong: E0 80 80 (U+0000 instead of U+0800+) */
  T(ccunicode_to_codepoint("\xE0\x80\x80", 3, &val) == 0);

  /* 3-byte overlong: E0 9F BF (U+07FF, fits in 2 bytes) */
  T(ccunicode_to_codepoint("\xE0\x9F\xBF", 3, &val) == 0);

  /* 4-byte overlong: F0 80 80 80 (U+0000) */
  T(ccunicode_to_codepoint("\xF0\x80\x80\x80", 4, &val) == 0);

  /* 4-byte overlong: F0 8F BF BF (U+FFFF, fits in 3 bytes) */
  T(ccunicode_to_codepoint("\xF0\x8F\xBF\xBF", 4, &val) == 0);
}
...
void test_decode_surrogate(void) {
  ...
  T(ccunicode_to_codepoint("\xED\xA0\x80", 3, &val) == 0);

  /* U+DFFF */
  T(ccunicode_to_codepoint("\xED\xBF\xBF", 3, &val) == 0);

  /* edge: U+D7FF (last before surrogates) — SHOULD be valid */
  T(ccunicode_to_codepoint("\xED\x9F\xBF", 3, &val) == 3
    && val == 0xD7FF);

  /* edge: U+E000 (first after surrogates) — SHOULD be valid */
  T(ccunicode_to_codepoint("\xEE\x80\x80", 3, &val) == 3
    && val == 0xE000);
}
...
void test_decode_invalid_first(void) {
  ...
  T(ccunicode_to_codepoint("\x80", 1, &val) == 0);
  T(ccunicode_to_codepoint("\xBF", 1, &val) == 0);

  /* out of range: 0xF5–0xFF (would produce > U+10FFFF) */
  T(ccunicode_to_codepoint("\xF5", 1, &val) == 0);
  T(ccunicode_to_codepoint("\xF8", 1, &val) == 0);
  T(ccunicode_to_codepoint("\xFF", 1, &val) == 0);

  /* 0xFE, 0xFF */
  T(ccunicode_to_codepoint("\xFE", 1, &val) == 0);
  T(ccunicode_to_codepoint("\xFF", 2, &val) == 0);
}
...
void test_decode_bad_continuation(void) {
  ...
  T(ccunicode_to_codepoint("\xC2\x00", 2, &val) == 0);

  /* 2-byte: C2 + ASCII (0x41) */
  T(ccunicode_to_codepoint("\xC2\x41", 2, &val) == 0);

  /* 3-byte: E4 B8 + non-continuation */
  T(ccunicode_to_codepoint("\xE4\xB8\x00", 3, &val) == 0);

  /* 3-byte: E4 + non-continuation + anything */
  T(ccunicode_to_codepoint("\xE4\x00\xAD", 3, &val) == 0);

  /* 4-byte: F0 + 3 continuations, but 2nd is bad */
  T(ccunicode_to_codepoint("\xF0\x9F\x00\x80", 4, &val) == 0);
}
...
void test_decode_truncated(void) {
  ...
  T(ccunicode_to_codepoint("\xC2", 1, &val) == 0);

  /* 3-byte leader with only 2 bytes available */
  T(ccunicode_to_codepoint("\xE4\xB8", 2, &val) == 0);

  /* 4-byte leader with only 3 bytes available */
  T(ccunicode_to_codepoint("\xF0\x9F\x98", 3, &val) == 0);

  /* 4-byte leader with only 1 byte available */
  T(ccunicode_to_codepoint("\xF0", 1, &val) == 0);

  /* 2-byte but len=1 */
  T(ccunicode_to_codepoint("\xC2\xA2", 1, &val) == 0);
}
...
void test_decode_null_edge(void) {
  ...
  /* NULL string */
  T(ccunicode_to_codepoint(NULL, 1, &val) == 0);

  /* NULL val */
  T(ccunicode_to_codepoint("A", 1, NULL) == 0);

  /* len = 0 */
  T(ccunicode_to_codepoint("A", 0, &val) == 0);

  /* len = -1 (should be caught) */
  T(ccunicode_to_codepoint("A", -1, &val) == 0);
}

/* ════════════════════════════════════════════════════════════════════════
**  ccunicode_from_codepoint  —  UCS-4 → UTF-8
** ════════════════════════════════════════════════════════════════════════ */
...
void test_encode_ascii(void) {
  ...
  T(ccunicode_from_codepoint(0x41, buf, &len) && len == 1 && buf[0] == 'A');
  T(ccunicode_from_codepoint(0x00, buf, &len) && len == 1 && buf[0] == '\x00');
  T(ccunicode_from_codepoint(0x7F, buf, &len) && len == 1 && buf[0] == '\x7F');
}
...
void test_encode_2byte(void) {
  ...
  /* U+00A2 (¢) = C2 A2 */
  T(ccunicode_from_codepoint(0xA2, buf, &len)
    && len == 2 && buf[0] == '\xC2' && buf[1] == '\xA2');

  /* U+00A9 (©) = C2 A9 */
  T(ccunicode_from_codepoint(0xA9, buf, &len)
    && len == 2 && buf[0] == '\xC2' && buf[1] == '\xA9');

  /* boundary: min 2-byte U+0080 = C2 80 */
  T(ccunicode_from_codepoint(0x80, buf, &len)
    && len == 2 && buf[0] == '\xC2' && buf[1] == '\x80');

  /* boundary: max 2-byte U+07FF = DF BF */
  T(ccunicode_from_codepoint(0x7FF, buf, &len)
...
void test_encode_3byte(void) {
  ...
  /* U+4E2D (中) = E4 B8 AD */
  T(ccunicode_from_codepoint(0x4E2D, buf, &len)
    && len == 3
  ...
  /* boundary: min 3-byte U+0800 = E0 A0 80 */
  T(ccunicode_from_codepoint(0x800, buf, &len)
    && len == 3
...
void test_encode_4byte(void) {
  ...
  /* U+1F600 (😀) = F0 9F 98 80 */
  T(ccunicode_from_codepoint(0x1F600, buf, &len)
    && len == 4
  ...
  /* boundary: max U+10FFFF = F4 8F BF BF */
  T(ccunicode_from_codepoint(0x10FFFF, buf, &len)
    && len == 4
...
void test_encode_invalid(void) {
  ...
  /* surrogate: U+D800 */
  T(ccunicode_from_codepoint(0xD800, buf, &len) == false);

  /* surrogate: U+DFFF */
  T(ccunicode_from_codepoint(0xDFFF, buf, &len) == false);

  /* beyond max: U+110000 */
  T(ccunicode_from_codepoint(0x110000, buf, &len) == false);

  /* well beyond: U+FFFFFFFF */
  T(ccunicode_from_codepoint(0xFFFFFFFF, buf, &len) == false);
}
...
void test_encode_null_edge(void) {
  ...
  /* NULL str */
  T(ccunicode_from_codepoint(0x41, NULL, &len) == false);

  /* NULL len */
  char buf[4];
  T(ccunicode_from_codepoint(0x41, buf, NULL) == false);

  /* both NULL */
  T(ccunicode_from_codepoint(0x41, NULL, NULL) == false);
}
...
void test_roundtrip(void) {
  ...
  for (int i = 0; i < n_cp; i++) {
    ...
    T(ccunicode_from_codepoint(orig, buf, &enc_len));

    uint32_t decoded; int dec_len;
    dec_len = ccunicode_to_codepoint(buf, enc_len, &decoded);
    T(dec_len == enc_len);
    T(decoded == orig);
  }
}
...
void test_fuzz_exhaustive(void) {
  ...
  for (uint32_t cp = 0x00; cp <= 0x7F; cp++) {
    T(ccunicode_from_codepoint(cp, buf, &enc_len));
    dec_len = ccunicode_to_codepoint(buf, enc_len, &decoded);
